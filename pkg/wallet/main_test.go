...
func TestMain(m *testing.M) {
	restore, err := isolateTestHome("sky10-wallet-home-")
	if err != nil {
...
func isolateTestHome(prefix string) (func(), error) {
	...
	oldSky10Home, hadSky10Home := os.LookupEnv(config.EnvHome)
	...
	if err := os.Setenv(config.EnvHome, filepath.Join(home, ".sky10")); err != nil {
		return nil, err
	}
	...
	return func() {
		...
		if hadSky10Home {
			_ = os.Setenv(config.EnvHome, oldSky10Home)
		} else {

...
class HermesClient:
    ...
    def ask(self, session_id: str, text: str) -> str:
        if self.use_responses_api:
            try:
                return self._ask_responses(session_id, text)
                ...
                if self._responses_api_unsupported(str(exc)):
                    ...
                    raise
        return self._ask_chat_completions(session_id, text)

    def _ask_responses(self, session_id: str, text: str) -> str:
        payload = {
            "model": self.model,
            "input": text,
        ...
        response = self._request("/responses", payload)
        result = self._extract_responses_text(response)
        return result or "Done."

    def _ask_chat_completions(self, session_id: str, text: str) -> str:
        ...
        if not choices:
            raise BridgeError("Hermes chat completions returned no choices")
        message = (choices[0] or {}).get("message") or {}
        answer = extract_text(message.get("content", ""))
        if not answer.strip():
            answer = "Done."
...
class Bridge:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                with urllib.request.urlopen(request, timeout=3600) as response:
                    ...
                    for event_name, data in self._iter_sse(response):
    ...
    def _handle_message(self, message: dict[str, Any]) -> None:
        ...
        with self._session_lock(session_id):
            ...
            try:
                reply = self.hermes.ask(session_id, user_text)
                if reply.strip():
                    self.sky10.send(sender, session_id, reply, sender, "text")
                    log(f"Reply sent for session {session_id}")
            except Exception as exc:
                error_text = f"Hermes bridge error: {exc}"
                log(error_text)
                try:
                    self.sky10.send(sender, session_id, error_text, sender, "error")

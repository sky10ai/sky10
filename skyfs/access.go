...
func GrantAccess(ctx context.Context, backend skyadapter.Backend, identity *Identity, namespace string, recipientPub ed25519.PublicKey) error {
...
func RevokeAccess(ctx context.Context, backend skyadapter.Backend, identity *Identity, namespace string, recipientPub ed25519.PublicKey) error {
	recipientID := shortID(recipientPub)
	recipientKeyPath := "keys/namespaces/" + namespace + "." + recipientID + ".ns.enc"

	// Delete recipient's wrapped key
	err := backend.Delete(ctx, recipientKeyPath)
	if err != nil && !errors.Is(err, skyadapter.ErrNotFound) {
...
func RotateNamespaceKey(ctx context.Context, backend skyadapter.Backend, identity *Identity, namespace string) error {
...
func ListAuthorizedKeys(ctx context.Context, backend skyadapter.Backend, namespace string) ([]string, error) {

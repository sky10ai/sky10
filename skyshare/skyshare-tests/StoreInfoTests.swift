...
struct StoreInfoTests {
    ...
    func decodeFull() throws {
        let json = """
        {"id":"sky10://k1_abc123","file_count":42,"total_size":1048576,"namespaces":["journal","financial"]}
        """
        let info = try JSONDecoder().decode(StoreInfo.self, from: json.data(using: .utf8)!)
        #expect(info.id == "sky10://k1_abc123")
    ...
    func decodeNullNamespaces() throws {
        let json = """
        {"id":"sky10://k1_abc","file_count":0,"total_size":0,"namespaces":null}
        """
        let info = try JSONDecoder().decode(StoreInfo.self, from: json.data(using: .utf8)!)
    ...
    func decodeMissingNamespaces() throws {
        let json = """
        {"id":"sky10://k1_abc","file_count":0,"total_size":0}
        """
        let info = try JSONDecoder().decode(StoreInfo.self, from: json.data(using: .utf8)!)
        #expect(info.namespaces == nil)
    }

...
- Useful RPC methods: `skyfs.deviceList`, `debug.list`,
  `debug.get`, `debug.dump`, `debug.requestScreenshot`,
  `debug.screenshot`, `skyfs.s3List`, `skyfs.s3Delete`.
- `debug.*` stores and reads local artifacts under the target
  daemon's `~/.sky10/debug/` directory, and mirrors to S3 when storage
  is configured.
- `debug.requestScreenshot` asks an open web UI to surface the
  RootAgent browser-capture flow; `debug.screenshot` stores the
  resulting browser-captured PNG plus page context. `debug.get` returns
  base64 for PNG keys.
- Legacy `skyfs.debugDump`, `skyfs.debugList`, `skyfs.debugGet`, and
  `skyfs.debugScreenshot` aliases are still supported.
